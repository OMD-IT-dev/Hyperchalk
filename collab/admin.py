...
class ExcalidrawLogRecordAdmin(admin.ModelAdmin):
    ...
    def view_json(self, obj: m.ExcalidrawLogRecord):
        if obj.pk:
            json_link = reverse("api-1:get_record", kwargs={"room_name": obj.room_name, "pk": obj.pk})
            return format_html(
                "<a href={json_link} target='_blank'>{text}</a>", json_link=json_link, text=_("Go to JSON")
            )
...
class ExcalidrawRoomAdmin(admin.ModelAdmin):
    ...
    def room_link(self, obj: m.ExcalidrawRoom):
        if obj.pk:
            ...
            return format_html(
                "<a href='{room_link}' target='_blank'>{text}</a>", room_link=room_link, text=_("Go to room")
            )
        return _("will be generated after saving")
    ...
    def room_json(self, obj: m.ExcalidrawRoom):
        if obj.pk:
            ...
            return format_html(
                "<a href='{room_link}' target='_blank'>{text}</a>", room_link=room_link, text=_("Go to room JSON")
            )
        return _("will be generated after saving")
    ...
    def replay_link(self, obj: m.ExcalidrawRoom):
        if obj.pk:
            ...
            return format_html(
                "<a href='{room_link}' target='_blank'>{text}</a>", room_link=room_link, text=_("Replay this room")
            )
        return _("will be generated after saving")
    ...
    def clone_rooms(self, request, queryset):
        ...
        self.message_user(
            request, _("Rooms created: %s") % ", ".join([r.room_name for r in new_rooms]), messages.SUCCESS
        )
