...
def reverse_ws_url(request: HttpRequest, route: Literal["replay", "collaborate"], room_name: str):
    return urlunsplit(
        ("wss" if request.is_secure() else "ws", request.get_host(), f"/ws/collab/{room_name}/{route}", None, None)
    )
...
async def index(request: HttpRequest, *args, **kwargs):
        ...
        if room_name := request.POST.get("roomname"):
            ...
            if created:
                ...
                return render(
                    ...
                    {
                        "imprint_url": settings.IMPRINT_URL,
                        "error_message": _("The room name “%(room_name)s” is already taken.")
                        % {"room_name": room_name},
                    },
                )
    ...
    if settings.ALLOW_AUTOMATIC_ROOM_CREATION:
        if settings.ALLOW_ANONYMOUS_VISITS or await user_is_authenticated(request.user):
            ...
            return redirect(reverse_with_query("admin:login", query_kwargs={"next": f"/{make_room_name(24)}/"}))
...
async def room(request: HttpRequest, room_name: str):
    ...
    room_obj, username = await asyncio.gather(
        async_get_object_or_404(m.ExcalidrawRoom, room_name=room_name), get_username(request.user)
    )
    ...
    is_lti_room, is_staff, file_dicts = await asyncio.gather(
        course_exists(room_obj), user_is_staff(request.user), get_file_dicts(room_obj)
    )
    ...
    return render(
        ...
        {
            "excalidraw_config": {
                "FILE_URL_TEMPLATE": absolute_reverse(
                    request, "api-1:put_file", kwargs={"room_name": room_name, "file_id": "FILE_ID"}
                ),
