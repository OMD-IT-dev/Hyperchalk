...
from .utils import (
    get_course_context,
    get_course_id,
    get_custom_launch_data,
    get_launch_url,
    get_lti_tool,
    get_room_name,
    get_user_from_launch_data,
    launched_by_superior,
...
async def register_consumer(request: HttpRequest, link: m.OneOffRegistrationLink):
    ...
    if not openid_config_endpoint:
        logger.warning(
            "a client tried to register but did not supply the proper parameters. The supplied " "parameters are:\n%s",
    ...
    async with aiohttp.ClientSession() as session:
        ...
        resp = await session.post(
            ...
            headers={"Authorization": "Bearer " + jwt_str, "Accept": "application/json"},
        )
    ...
    logging.info('Registration of issuer "%s" with client %s complete', consumer.issuer, consumer.client_id)
...
def oidc_login(request: HttpRequest):
    ...
    oidc_redirect = oidc_login_handle.enable_check_cookies().redirect(target_link_uri, False)
...
def lti_configure(request: HttpRequest, launch_id: str):
    ...
    title = message_launch_data.get("https://purl.imsglobal.org/spec/lti-dl/claim/deep_linking_settings", {}).get(
        "title", course_title
    )
    ...
    if mode == Modes.CLASSROOM:
        ...
        if n_groups > 50:
            logger.warning(
                _("User %s tried to create a board with more than %d groups"), user.username, settings.MAX_GROUPS
            )
            return HttpResponseForbidden(_("There can't be more than %d groups.") % (settings.MAX_GROUPS))
        ...
        if len(existing_groups) > n_groups:
            ...
            return HttpResponseBadRequest(
                _("The number of groups you specified " "is smaller than the groups that currenty exist.")
                % (settings.MAX_GROUPS)
            )

        room_names = [make_room_name(24) for _ in range(n_groups - len(existing_groups))]
        upsert_room_mappers(room_names)
        resource.set_title(title + " – " + Modes.GROUPWORK.label).set_custom_params(
            {"mode": mode, "rooms": ",".join(room_names)}
        )

    elif mode == Modes.STUDENT:
        resource.set_title(title + " – " + Modes.STUDENT.label).set_custom_params(
            {"mode": mode, "room": make_room_name(24)}
        )
    ...
    if message_launch.is_resource_launch():
        ...
        if mode == Modes.GROUPWORK:
            ...
            room_pointers = [
                CourseToRoomMapper.objects.get_or_create_for_course(
                    lti_data_room=room_pointer_id, course_id=course_id, user=user, mode=mode, lti_tool=lti_tool
                )
                for room_pointer_id in custom_data.get("rooms").split(",")
            ]
            return render(
                request, "ltiapi/choose_group.html", {"room_pointers": [pointer for pointer, created in room_pointers]}
            )
        ...
        if launched_by_superior(message_launch_data) and mode in [Modes.STUDENT, Modes.STUDENT_LEGACY]:
            ...
            if mode == Modes.STUDENT:
                room_pointers = CourseToRoomMapper.objects.filter(
                    lti_data_room=lti_data_room, course_id=course_id, mode=mode
                )
            elif mode == Modes.STUDENT_LEGACY:
                room_pointers = CourseToRoomMapper.objects.filter(
                    lti_data_room__startswith=lti_data_room, course_id=course_id, mode=mode
                )
            return render(request, "ltiapi/board_teacher_overview.html", {"room_pointers": room_pointers})
        ...
        if mode in [Modes.CLASSROOM, Modes.STUDENT, Modes.STUDENT_LEGACY]:
            ...
            room_pointer, __ = CourseToRoomMapper.objects.get_or_create_for_course(
                lti_data_room=lti_data_room, course_id=course_id, user=user, mode=mode, lti_tool=lti_tool
            )
            ...
            if not async_to_sync(user_is_authorized)(user, room, request.session):
                logger.warning("User %s tried to join room (%s) %s, " "despite not being authorized.", user, mode, room)
