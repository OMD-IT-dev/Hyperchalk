...
async def make_tool_config_from_openid_config_via_link(
    openid_config: Dict[str, Any], openid_registration: Dict[str, Any], one_off_registration: m.OneOffRegistrationLink
):
...
def get_course_context(message_launch_data: dict):
    return message_launch_data.get(f"{CLAIM}/context", {})


def get_custom_launch_data(message_launch_data: dict):
    return message_launch_data.get(f"{CLAIM}/custom", {})


def get_ext_data(message_launch_data: dict):
    return message_launch_data.get(f"{CLAIM}/ext", {})
...
def get_room_name(request: HttpRequest, message_launch_data: dict):
    return get_custom_launch_data(message_launch_data).get("room", None) or request.GET.get("room")
...
def get_user_from_launch_data(message_launch_data: dict, lti_tool: LtiTool):
    ...
    user, __ = m.CustomUser.objects.get_or_create(
        username=username, defaults={"first_name": user_full_name, "registered_via": lti_tool}
    )
